#I lost the source of this code but i'm currently looking for it
import numpy
import sys
...
reading = '''The Project Gutenberg eBook of Frankenstein, by Mary Wollstonecraft (Godwin) Shelley
...
con = sqlite3.connect('NLP.db')

cur = con.cursor()

cur.execute('''CREATE TABLE if not exists Zinnen(id integer primary key, zin text not null, aantal text not null)''')
...
model.fit(X, y, epochs=20, batch_size=2560, callbacks=desired_callbacks)
...
#i save the sentence in a databse
zin= ("insert into Zinnen(id, zin, aantal) values({0},'{1}','{2}')")
epochs = ' epochs=20, batch_size=256 // ramge 256'
final = zin.format(2, result, epochs)
print(final)
cur.execute(final)